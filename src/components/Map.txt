  ...
  return (
    <div className={classNames("min-h-screen", themeColors[theme])}>
      <section className={isActiveModalNavbar ? "relative opacity-60 -z-50 mt-24" : "flex flex-col z-50 mt-24"}>
        ...
        <MapContainer center={[4.16934, 10.632949]} zoom={13} style={{ height: "80vh", width: "100%", minHeight: "80vh" }}>
          <TileLayer url={tileOptions[tileType]} />
          {geojsonData && <GeoJSON
  key={JSON.stringify(geojsonData.map(f => f.properties.__highlighted))} // force remount when highlights change
  data={geojsonData}
  style={getStyle}
...
{limitGeojsonData && (
  <GeoJSON 
    ...
    interactive={false}
    pane="shadowPane"  // <-- Renders below features
  />
)}
        </MapContainer>
